...
import { PfcTransactionSchema, type PfcTransaction } from './data';
...
const PfcStatementUploadSchema = z.object({
  fileName: z.string(),
  transactions: z.array(PfcTransactionSchema),
});

export async function savePfcStatement(data: z.infer<typeof PfcStatementUploadSchema>) {
  await requireAdminAuth();
  const validation = PfcStatementUploadSchema.safeParse(data);
  if (!validation.success) {
      console.error("Invalid PFC statement data:", validation.error.flatten());
  ...
  try {
    ...
    if (transactions.length > 0) {
        ...
        const pfcAccountRef = cashAccountsCollection.doc('pfc_account');
        batch.set(pfcAccountRef, {
            bankName: 'PFC Account',
    ...
    await logActivity('UPLOAD_BANK_STATEMENT', {
      entityType: 'PFC Account Statement',
    ...
    revalidatePath('/pfc-account-statement');
    revalidatePath('/other-expenses');
    revalidatePath('/other-income');
    revalidatePath('/apps-received-payments'); // Revalidate the new path
    ...
    console.error("Error saving PFC statement:", error);
    return { error: 'Failed to save statement to database.' };
  }
}

export async function getExistingPfcTransactionIds(transactionIds: string[]): Promise<string[]> {
    await requireAdminAuth();
    ...
    try {
        ...
        console.error("Error fetching existing PFC transaction IDs:", error);
