...
export function AndroidAppsDataTable<TData, TValue>({
  columns,
  data,
  isLoading,
    ...
    <div className="min-w-0 overflow-hidden rounded-2xl border border-white/10 bg-card/80 shadow-sm">
      <div className="flex flex-col gap-3 border-b border-white/10 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <h2 className="text-sm font-semibold text-foreground">Android Apps</h2>
          <p className="mt-0.5 text-xs text-muted-foreground">
            {isLoading ? 'Loading app registry...' : `${data.length} live app(s) available for Android settings`}
          </p>
        </div>
              ...
              <TableRow className="border-white/10 hover:bg-transparent">
                <TableCell colSpan={columns.length} className="h-44 text-center">
                  <div className="mx-auto flex max-w-sm flex-col items-center gap-3 text-muted-foreground">
                    ...
                    <div>
                      <div className="font-medium text-foreground">No Android apps found</div>
                      <p className="mt-1 text-xs">Check the Apps registry and make sure the platform contains Android.</p>
                    </div>
                  </div>
                </TableCell>
              </TableRow>
