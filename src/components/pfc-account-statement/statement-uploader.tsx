...
import type { PfcTransaction } from '@/app/(dashboard)/pfc-account-statement/data';
import { savePfcStatement, getExistingPfcTransactionIds } from '@/app/(dashboard)/pfc-account-statement/actions';
import { StatementTable } from './statement-table';
import Papa from 'papaparse';

interface StatementUpload {
    fileName: string;
    transactions: PfcTransaction[];
}

type RawPfcTransaction = Omit<PfcTransaction, 'category' | 'isDuplicate'>;
...
export function StatementUploader() {
  ...
  const parseCsv = async (csvContent: string, fileName: string): Promise<StatementUpload> => {
    ...
    const rawTransactions: RawPfcTransaction[] = (parsed.data as any[]).map((row): RawPfcTransaction | null => {
    ...
    }).filter((t): t is RawPfcTransaction => t !== null);
    ...
    const existingIds = await getExistingPfcTransactionIds(transactionIds);
  ...
  const handleSave = async () => {
    ...
    const result = await savePfcStatement(parsedData);
      ...
      case 'idle':
        return (
          <div {...getRootProps()} className={`flex flex-col items-center justify-center p-12 border-2 border-dashed rounded-lg cursor-pointer transition-colors ${isDragActive ? 'border-primary bg-accent/20' : 'border-border hover:border-primary/50'}`}>
            ...
            <p className="font-semibold">Drag & drop your PFC statement CSV here</p>
