...
function admob_cron_env($name, $default = '')
{
    $value = getenv($name);
    return ($value === false || $value === '') ? $default : $value;
}

function admob_global_setting(string $name): string
{
    global $main_mysqli;

    if (!isset($main_mysqli) || !$main_mysqli instanceof mysqli) {
        return '';
    }
    ...
    return trim((string)($result[0]['string_value'] ?? ''));
}

function admob_config_value(string $envName, string $settingName, string $default = ''): string
{
    ...
    $settingValue = admob_global_setting($settingName);
    if ($settingValue !== '') {
        return $settingValue;
    }
...
function admob_cron_json_response(array $payload, int $statusCode = 200): void
{
...
function admob_cron_request(string $url, array $options = []): array
{
    $ch = curl_init($url);
...
function admob_get_access_token(): string
{
    $clientId = admob_config_value('ADMOB_CLIENT_ID', 'admob_client_id', ADMOB_CLIENT_ID_FALLBACK);
    $clientSecret = admob_config_value('ADMOB_CLIENT_SECRET', 'admob_client_secret', ADMOB_CLIENT_SECRET_FALLBACK);
    $refreshToken = admob_config_value('ADMOB_REFRESH_TOKEN', 'admob_refresh_token', ADMOB_REFRESH_TOKEN_FALLBACK);

    if ($clientId === '' || $clientSecret === '' || $refreshToken === '') {
        throw new Exception('ADMOB_CLIENT_ID, ADMOB_CLIENT_SECRET, and ADMOB_REFRESH_TOKEN env vars are required.');
    }

    $response = admob_cron_request('https://oauth2.googleapis.com/token', [
...
function admob_get_app_platform_map(mysqli $mysqli): array
{
    $map = [];
    $sql = "
    ...
    while ($row = $result->fetch_assoc()) {
        $map[(string)$row['admob_app_id']] = strtolower((string)$row['platform']);
    }
...
function admob_normalize_platform(string $platform): string
{
...
function admob_resolve_platform(string $admobAppId, array $idPlatformMap): string
{
    if (isset($idPlatformMap[$admobAppId])) {
        return admob_normalize_platform($idPlatformMap[$admobAppId]);
    }

    return 'unknown';
}

function admob_sync_daily_report(mysqli $mysqli, string $date, bool $debug = false): array
{
    $publisherId = admob_config_value('ADMOB_PUBLISHER_ID', 'admob_publisher_id', ADMOB_PUBLISHER_ID_FALLBACK);
    if ($publisherId === '') {
        throw new Exception('ADMOB_PUBLISHER_ID env var is required. Example: pub-1234567890123456');
    }
    ...
    $accessToken = admob_get_access_token();
    $platformMap = admob_get_app_platform_map($mysqli);
    ...
    $url = 'https://admob.googleapis.com/v1/accounts/' . rawurlencode($publisherId) . '/networkReport:generate';
    $response = admob_cron_request($url, [
    ...
    $responseItems = admob_parse_report_response($response['body']);
    $rows = [];
    foreach ($responseItems as $payload) {
        ...
        $platform = admob_resolve_platform($admobAppId, $platformMap);
        $impressions = (int)($metricValues['IMPRESSIONS']['integerValue'] ?? 0);
        $clicks = (int)($metricValues['CLICKS']['integerValue'] ?? 0);
        $earningsMicros = (int)($metricValues['ESTIMATED_EARNINGS']['microsValue'] ?? 0);
        $earningsUsd = round($earningsMicros / 1000000, 6);
...
function admob_parse_report_response(string $body): array
{
    $trimmed = trim($body);
    if ($trimmed === '') {
        return [];
...
function admob_sync_date_range(mysqli $mysqli, string $fromDate, string $toDate, bool $debug = false): array
{
    ...
    while ($from <= $to) {
        $result = admob_sync_daily_report($mysqli, $from->format('Y-m-d'), $debug);
        $results[] = $result;
...
function admob_month_range(string $month): array
{
...
try {
    ...
    if ($month) {
        [$fromDate, $toDate] = admob_month_range($month);
    }

    if ($fromDate && $toDate) {
        admob_cron_json_response([
            'success' => true,
            'result' => admob_sync_date_range($main_mysqli, $fromDate, $toDate, $debug),
        ]);
    }
    ...
    admob_cron_json_response([
        'success' => true,
        'result' => admob_sync_daily_report($main_mysqli, $date, $debug),
    ]);
