...
// Fill these values before adding this file to cron.
...
const ADMOB_CLIENT_ID = '';
const ADMOB_CLIENT_SECRET = '';
const ADMOB_REFRESH_TOKEN = '';
const ADMOB_PUBLISHER_ID = ''; // Example: pub-1234567890123456
...
try {
    ...
    if ($month) {
        $cronHistoryMode = 'month_override';
        [$fromDate, $toDate] = admob_standalone_month_range($month);
    }

    if ($fromDate && $toDate) {
        if ($cronHistoryMode === 'daily') {
            $cronHistoryMode = 'date_range';
        }
        ...
        $stats = report_cron_history_payload_stats($payload);
        report_cron_history_record($mysqli, [
            'job_key' => 'sync_admob_daily_standalone',
            'job_name' => 'AdMob Daily Sync',
            'platform' => 'admob',
            'report_type' => 'daily',
            'status' => 'success',
            'mode' => $cronHistoryMode,
        ...
        admob_standalone_json_response($payload);
    }
    ...
    admob_standalone_json_response($payload, 500);
}
